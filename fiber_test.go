...
func TestCtx(t *testing.T) {
	...
	request := httptest.NewRequest("GET", "/hello?name=Yusuf", nil)
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Hello Yusuf", string(bytes))
...
func TestHttpRequest(t *testing.T) {
	...
	request.Header.Set("firstname", "Yusuf")
	request.AddCookie(&http.Cookie{Name: "lastname", Value: "Supriadi"})
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Hello Yusuf Supriadi", string(bytes))
}
...
func TestRouteParameter(t *testing.T) {
	...
	request := httptest.NewRequest("GET", "/users/yusuf/orders/10", nil)
	response, err := app.Test(request)
	assert.Nil(t, err)
	assert.Equal(t, 200, response.StatusCode)
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, "Get Order 10 From User yusuf", string(bytes))
}
...
func TestFormRequest(t *testing.T) {
	...
	body := strings.NewReader("name=Yusuf")
...
func TestRequestBody(t *testing.T) {
	...
	body := strings.NewReader(`{"username":"Yusuf", "password":"password"}`)
...
func TestBodyParserForm(t *testing.T) {
	...
	body := strings.NewReader(`username=Yusuf&password=rahasia&name=Yusuf+Supriadi`)
	request := httptest.NewRequest("POST", "/register", body)
	request.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	response, err := app.Test(request)
	assert.Nil(t, err)
...
func TestResponseJSON(t *testing.T) {
	...
	bytes, err := io.ReadAll(response.Body)
	assert.Nil(t, err)
	assert.Equal(t, `{"name":"Firlana Luchiana","username":"luchiana"}`, string(bytes))
}
